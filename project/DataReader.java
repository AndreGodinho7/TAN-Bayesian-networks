...
public interface DataReader {
	...
	class ArrayData implements DataReader {
		int[][] featureMatrix;
		int[] classArray;
		...
		public void readFile() {
			...
			while (inputStream.hasNext()) {
				...
				/*for (int fileColumn = 0; fileColumn < values.length - 1; fileColumn++) {
					featureMatrix[fileRow][fileColumn] = Integer.parseInt(values[fileColumn]);					
				}*/
				this.featureMatrix[fileRow] = values;
				this.classArray[fileRow] = values[values.length - 1];
				//classArray[fileRow] = Integer.parseInt(values[values.length - 1]);
				fileRow++;
			}
			inputStream.close();
		}
	...
	/*class ArrayListData implements DataReader {
